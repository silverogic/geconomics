...
export const translations = {
  en: {
    ...
    // Hero Section
    heroBadge: 'Official World Bank Open Data × Real-time Forex Feed',
    heroTitle: 'Real-time Global Forex & GDP Analysis',
    heroDescription:
      'A zero-database, on-demand economic intelligence platform. Directly queries authoritative international public APIs to deliver accurate macroeconomic indicators and real-time exchange rates.',
    ...
    modalAccuracyGdp:
      'GDP Statistics: Sourced directly from World Bank Open Data (Indicators: Total GDP NY.GDP.MKTP.CD, GDP Per Capita NY.GDP.PCAP.CD).',
    modalAccuracyFx:
    ...
    compareDescription:
      'Compare World Bank official GDP historical trends and real-time exchange rates side by side.',
    ...
    footerZeroStorageDesc:
      'This website operates with zero persistent backend storage or databases. Data is retrieved on-demand directly by your browser from official international endpoints.',
    footerDisclaimer:
  ...
  ko: {
    ...
    // Hero Section
    heroBadge: '세계은행(World Bank) 공식 데이터 × 실시간 외환 시장 연동',
    heroTitle: '전 세계 국가별 환율 & GDP 실시간 분석',
    heroDescription:
      '데이터 보관 없이 브라우저에서 직접 온디맨드로 조회하는 공신력 있는 글로벌 경제 데이터 플랫폼입니다. 국가를 클릭하여 10개년 시계열 추이와 실시간 환율 계산기를 확인하세요.',
    ...
    modalAccuracyGdp:
      'GDP 통계: 세계은행 Open Data API(총 GDP NY.GDP.MKTP.CD, 1인당 GDP NY.GDP.PCAP.CD) 공식 집계치 기준.',
    modalAccuracyFx:
    ...
    compareDescription:
      '두 국가의 세계은행 공식 GDP 추이와 실시간 환율을 직관적으로 나란히 대조합니다.',
    ...
    footerZeroStorageDesc:
      '본 웹사이트는 별도의 서버나 데이터베이스 없이 사용자의 웹 브라우저에서 공식 API(세계은행 및 글로벌 외환 시장)로 직접 연결되어 최신 데이터를 실시간으로 온디맨드 렌더링합니다.',
    footerDisclaimer:
